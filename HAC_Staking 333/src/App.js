...
export default function NFT() {
  ...
  async function callApi() {
    ...
    const apicall = await Promise.all(nfts.map(async i => {
      ...
      let fetchstkRes = await fetch(nftunstkURI).then(res => res.json()).then((d) =>  { let cleanUri = d.image.replace("ipfs://","https://ipfs.io/ipfs/"); let nftstkobj = [{tokenid: d.id, image:cleanUri}]; setFresh(nftstkobj)})
       return fetchstkRes
      }))
     ...
     const nftstkURI = await contract.methods.tokenURI(stkid.tokenId).call()
     let fetchRes = await fetch(nftstkURI).then(res => res.json()).then((d) =>  { let cleanUri = d.image.replace("ipfs://","https://ipfs.io/ipfs/"); let nftstkobj = [{tokenid: d.id, image:cleanUri}]; setFruits(nftstkobj)})
      return fetchRes
      }))
      getNfts(apicall)
      getStk(nftstk)
    ...
    return(
      <div className='App'>
        <div>
          ...
          <a href='https://horror-ape.club' ><img src='./logo333.png' alt="logo" width='35%' /></a>
          </div>
        </div>
                 ...
                 {fresh.map((nft, i) => {
                    ...
                    return (
                      <div className="card nft-card mt-3 mb-3" key={i} >
                        <div className="image-over">
                          <img className="card-img-top" src={nft.image} alt="unstaked-nifty" />
                        </div>
                    ...
                    {fruits.map((index, i) => {
                      async function unstakeit() {
                        vaultcontract.methods.unstake([index.tokenid]).send({ from: account });
                       let myInterval = setInterval(callApi(),20000);
                        clearInterval(myInterval);
                      }
                      return (
                        ...
                        <div className="card stakedcard mt-3 mb-3" key={i} >
                          <div className="image-over">
                            <img className="card-img-top" src={index.image} alt="image" />
                          </div>
                          <div className="card-caption col-12 p-0">
                            <div className="card-body">
                              <h5 className="mb-0">HAC 3333 NFT Collection #{index.tokenid}</h5>
                              <h5 className="mb-0 mt-2">STATUS:<p style={{ color: "#15F4EE", fontWeight: "bold", textShadow: "1px 1px 2px #000000" }}>Currently Staked</p></h5>
                              <div className="card-bottom d-flex justify-content-between">
                                <input key={i} type="hidden" id='stakeid' value={index.tokenid} />
