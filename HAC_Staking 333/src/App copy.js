...
export default function NFT() {
  ...
  const [nftUnstk, setNftUnstk] = useState([]);
  const [nftStk, getNftStk] = useState([]);
  ...
  async function callApi() {
    ...
    var approval = await contract.methods
      .isApprovedForAll(account, STAKINGCONTRACT)
      .call();
    rewardinfo();
    if (approval == false) {
    ...
    const nfts = await axios
      .get(
        moralisapi +
          `${account}/nft/0x7E82F54578f03ad922F2bA61674C46b9dBdCE61d?chain=bsc&format=decimal`,
        { headers: config }
      )
      .then((output) => {
    ...
    const apicall = await Promise.all(
      nfts.map(async (i) => {
        ...
        var unstakeduri = await contract.methods.tokenURI(item.tokenId).call();
        fetch(unstakeduri)
          .then((res) => res.json())
          .then((out) => {
            let cleanUri = out.image.replace(
              "ipfs://",
              "https://ipfs.io/ipfs/"
            );
            let meta = {
            ...
            unstknftsarr.push(meta);
            console.log(unstknftsarr);
          });
        return item;
      })
    );
    ...
    const nftstk = await Promise.all(
      stakednfts.map(async (i) => {
        ...
        var stakakeduri = await contract.methods.tokenURI(stkid.tokenId).call();
        fetch(stakakeduri)
          .then((res) => res.json())
          .then((out) => {
            let cleanUri = out.image.replace(
            ...
            let meta = {
              tokenID: stkid.tokenId,
              image: cleanUri,
            };
            stknftsarr.push(meta);
          });
        return stkid;
      })
    );
    getNfts(apicall);
    setNftUnstk(unstknftsarr);
    getNftStk(stknftsarr);
    ...
    <div className="App">
      <div>
        ...
        <div className="col" style={{ color: "#5c4673" }}>
          ...
          <h1 id="main-header"> 333 STAKING</h1>
        </div>
      </div>
      <div
        className="card mx-auto text-center"
        id="wrapper"
        style={{ width: "80vw", marginTop: "40px", backgroundColor: "black" }}
      >
        <h2 className="card-title mt-3" style={{ color: "yellow" }}>
          Total NFT's Staked: <br />{" "}
          <span style={{ color: "yellowgreen" }} id="total-staked"></span>
        </h2>
        <div
          className="card mx-auto text-center"
          id="item"
        ...
        >
          ...
          <div className="card-title mt-3" style={{ color: "yellow" }}>
            NFTs Staked:{" "}
            <span style={{ color: "yellowgreen" }} id="my-staked"></span>
          </div>
          <div className="card-title mt-3" style={{ color: "yellow" }}>
            Unclaimed Rewards:{" "}
            <span style={{ color: "yellowgreen" }} id="earned"></span>
          </div>
        </div>
        ...
        >
          Please click the stake button to approve
        </h6>
      ...
      <div
        className="card mx-auto text-center mb-3"
        style={{ backgroundColor: "#5c4673", width: "80%" }}
      ></div>
      <div className="col">
      ...
      <Button className="btn btn-warning" onClick={refreshNft}>
        Refresh
      </Button>
                     ...
                     (
                      <div className="card nft-card mt-3 mb-3" key={i} >
                        <div className="image-over">
                          <img className="card-img-top" src={nft.image} alt="nft-unstaked" />
                          <p>{nft.image}</p>
                        </div>
                        <div className="card-caption col-12 p-0">
                          <div className="card-body" >
                            <h5 className="mb-0">HAC 333 NFT Collection #{nft.tokenID}</h5>
                            ...
                            <div className="card-bottom d-flex justify-content-between">
                              ...
                              <Button className='btn btn-info mx-auto text-center' onClick={stakeit}>Stake it</Button>
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            {nftUnstk.map((nft, i) => {
                      ...
                      return (
                        ...
                        <div className="card stakedcard mt-3 mb-3" key={i} >
                          <div className="image-over">
                            <img className="card-img-top" src={nft.image} alt="nft-staked" />
                          </div>
                          <div className="card-caption col-12 p-0">
                            <div className="card-body">
                              <h5 className="mb-0">HAC 333 NFT Collection #{nft.tokenID}</h5>
                              ...
                              <div className="card-bottom d-flex justify-content-between">
                                <input key={i} type="hidden" id='stakeid' value={nft.tokenID} />
                                <Button className='btn btn-info mx-auto text-center' onClick={unstakeit}>Unstake it</Button>
                              </div>
                            </div>
                          </div>
                        </div>
